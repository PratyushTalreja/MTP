                   ...
                   )
{
  ...
  {
    iterate_prosail <- function(param, rsoil)
    {
      spec <- PROSAIL(N        = param[1],
                      Cab      = param[2],
                      Car      = param[3],
                      Cbrown   = param[4],
                      Cw       = param[5],
  ...
  #write(extern$reflectance, file = "/home/pratyush/Desktop/MTP - Phase2/Prosail/hsdar/LUT.txt", sep = "\n")
  spec <- speclib(wavelength=c(1:nw)+399,
                  spectra=extern$reflectance
                 )
  #return(spec)
  return(extern$reflectance)
}